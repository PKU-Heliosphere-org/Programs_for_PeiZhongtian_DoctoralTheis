; $Id: //depot/idl/IDL_64/idldir/lib/interpol.pro#1 $
;
...
FUNCTION INTERPOL_NearBy_MacOS, V, X, U, SPLINE=spline, LSQUADRATIC=ls2, QUADRATIC=quad, $
			Linear=linear, NearBy=NearBy
;+
; NAME:
;	INTERPOL
;
...
KEYWORD_SET(spline): BEGIN
    ...
    END
KeyWord_Set(linear): Begin                 ;Linear, not regular
	p = (u-x[s])*(v[s+1]-v[s])/(x[s+1] - x[s]) + v[s]
	End
KeyWord_Set(nearby): Begin
    p = replicate(v[0], n_elements(s)) ;Result
	sub_vect_tmp	= (u-x[s])/(x[s+1]-x[s])
	sub_lt_0	= Where(sub_vect_tmp lt 0.0)
	sub_gt_1	= Where(sub_vect_tmp gt 1.0)
	sub_normal	= Where(sub_vect_tmp ge 0.0 and sub_vect_tmp le 1.0)
	If (sub_lt_0[0] ne -1) Then Begin
		p[sub_lt_0]	= v[0]
	EndIf
	If (sub_gt_1[0] ne -1) Then Begin
		p[sub_gt_1]	= v[N_Elements(v)-1]
	EndIf
	p[sub_normal]	= v[s]*(0.^Round(sub_vect_tmp[sub_normal]))+$
						v[s+1]*Round(sub_vect_tmp[sub_normal])
	End
ENDCASE
