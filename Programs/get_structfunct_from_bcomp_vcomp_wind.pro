;Pro get_StructFunct_from_BComp_VComp_WIND
...
If i_BComp eq 1 Then Begin
  BComp_RTN_vect  = Vba_x;reform(Bxyz_GSE_2s_arr(0,*))
EndIf
If i_BComp eq 2 Then Begin
  BComp_RTN_vect  = Vba_y;reform(Bxyz_GSE_2s_arr(1,*))
EndIf
If i_BComp eq 3 Then Begin
  BComp_RTN_vect  = Vba_z;reform(Bxyz_GSE_2s_arr(2,*))
EndIf
...
;;;---
is_log    = 1
Diff_BComp_2_arr  = Fltarr(num_times, num_periods)
jieshu = 2.0
get_StructFunct_of_BComp_time_scale_arr_Helios, $
    ...
    BComp_SF_2_arr, $      ;output
    period_vect=period_vect, $
    Diff_BComp_arr=Diff_BComp_2_arr

num_periods = N_Elements(period_vect)
BComp_SF_2_vect  = Fltarr(num_periods)
num_times_vect      = Lonarr(num_periods)
For i_period=0,num_periods-1 Do Begin
  BComp_StructFunct_tmp = Mean(Reform(BComp_SF_2_arr(*,i_period)), /NaN)
  BComp_SF_2_vect(i_period)  = BComp_StructFunct_tmp
  sub_tmp = Where(Finite(Reform(BComp_SF_2_arr(*,i_period))) ne 0)
...
;;;---
is_log    = 1
Diff_BComp_4_arr  = Fltarr(num_times, num_periods)
jieshu = 4.0
get_StructFunct_of_BComp_time_scale_arr_Helios, $
    time_vect, BComp_vect, $    ;input
    jieshu,                   $       ;input
    period_range=period_range, $  ;input
    num_periods=num_periods, is_log=is_log, $ ;input
    BComp_SF_4_arr, $      ;output
    period_vect=period_vect, $
    Diff_BComp_arr=Diff_BComp_4_arr

num_periods = N_Elements(period_vect)
BComp_SF_4_vect  = Fltarr(num_periods)
num_times_vect      = Lonarr(num_periods)
For i_period=0,num_periods-1 Do Begin
  BComp_StructFunct_tmp = Mean(Reform(BComp_SF_4_arr(*,i_period)), /NaN)
  BComp_SF_4_vect(i_period)  = BComp_StructFunct_tmp
  sub_tmp = Where(Finite(Reform(BComp_SF_4_arr(*,i_period))) ne 0)
  num_times_tmp = N_ELements(sub_tmp)
  num_times_vect(i_period)  = num_times_tmp
EndFor
;;--
...
file_save = FileName_BComp+'_StructFunct'+'_arr'+$
        TimeRange_str+$
        PeriodRange_str+$
        file_version+$
        '.sav'
JulDay_min  = Min(JulDay_2s_vect)
data_descrip= 'got from "get_StructFunct_from_BComp_VComp_WIND.pro"'
Save, FileName=dir_save+file_save, $
  data_descrip, $
...
Bxyz_GSE_2s_arr = Vxyz_GSE_p_arr
JulDay_2s_vect = JulDay_vect
