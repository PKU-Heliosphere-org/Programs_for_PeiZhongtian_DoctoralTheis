;pro random_pdf_deltab_taucd_v2
...
;;---把耗散揉进去
xin_Bz_N   = fltarr(n_cell,num_periods)+!values.f_nan
rem_Bz_N   = fltarr(n_cell,num_periods)+!values.f_nan

for i_period = 0,2 do begin
  for i_cell = 0,n_cell-1 do begin
    rem_Bz_N(i_cell,i_period) = diff_Bz_N(i_cell,i_period)*tauc_taud(i_cell,i_period);exp(-tauc_taud(i_cell,i_period))
    xin_Bz_N(i_cell,i_period) = diff_Bz_N(i_cell,i_period)-rem_Bz_N(i_cell,i_period)
  endfor
endfor
...
;  XTicks=xticks,XTickV=xtickv,XTickName=xticknames,XMinor=xminor,XTickLen=0.04,$
  XTitle=xtitle,YTitle=ytitle,title=title, $
  Color=color_black,$
  /NoErase,Font=-1,CharThick=1.0,charsize=1.5,Thick=1.0,/xlog,/ylog
;             
;           
;;;;;;;;;;;;;;;;;;;;;;;;;
   i_x_SubImg  = 0
...
file_save ='random_deltaB_dissipation_'+strfang+'_'+string(alpha)+s_v+'_v2.sav'
...
file_version= '(v1)'
file_fig  = 'random_deltaB_PDF_alpha_taucd_'+strfang+'_'+string(alpha)+s_v+'_v2.png';;;;;;
