...
Pro subroutine_plot_Figure10_for_paper_200206, $
...
If is_png_eps eq 1 Then Begin
  ...
  xsize=900.0 & ysize=850.0
  Window,10,XSize=xsize,YSize=ysize,Retain=2
EndIf
EndElse
...
For i_subimg_y=0,1 Do Begin
  If  i_subimg_y eq 1 Then Begin ;ky for Zp with min eigen-value
    image_TV  = theta_zp_zm_arr
    title = TexToIDL('\theta (Z+,Z-)')
    PanelMark = '(a)'
  EndIf
 If i_subimg_y eq 0 Then Begin ;ky for Zp with min eigen-value
    image_TV  = theta_kp_km_arr
    title = TexToIDL('\theta (k+,k-)')
    PanelMark = '(b)'
  EndIf
