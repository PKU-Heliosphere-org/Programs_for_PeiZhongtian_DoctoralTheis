...
PRO TVIMAGE, image, x, y, $
   ...
   OVERPLOT=overplot, $
   TV=tv, $
   xywindow_posi=xywindow_posi,$	;added by JSHEPT on Jan.2,2006, contain the information of position of the window in a panel which the image is TVed
   _EXTRA=extra
...
ENDIF ELSE BEGIN
   IF Keyword_Set(multi) AND (Keyword_Set(overplot) NE 1)THEN BEGIN
   	;;*******************************
   	;;the following statements added by JSHEPT is used to keep the figure in its original shape while not distorts
		xmargin=[0.1,0.1]	&	ymargin=xmargin	&	ymargin=(ymargin/!d.y_ch_size)*!d.x_ch_size		;added by JSHEPT
			Plot, Findgen(11), XStyle=4, YStyle=4, /NoData, Background=background, XMargin=xmargin, YMargin=ymargin	;added by JSHEPT
			position(0)	= !x.window(0)+position(0)*(!x.window(1)-!x.window(0))							;added by JSHEPT
			position(2)	= !x.window(0)+position(2)*(!x.window(1)-!x.window(0))							;added by JSHEPT
			position(1)	= !y.window(0)+position(1)*(!y.window(1)-!y.window(0))							;added by JSHEPT
			position(3)	= !y.window(0)+position(3)*(!y.window(1)-!y.window(0))							;added by JSHEPT
			xywindow_posi	= [!x.window(0),!y.window(0),!x.window(1),!y.window(1)]	;very important when !p.multi is not 0, it is used to keep the original window-size/position information avoid of being sweeped out by 'plot' in following steps;
		;a	Plot, Findgen(11), XStyle=4, YStyle=4, /NoData, Background=background, XMargin=vmargin, YMargin=vmargin	;annotated by JSHEPT
   	;a	position = [!X.Window[0], !Y.Window[0], !X.Window[1], !Y.Window[1]]													;annotated by JSHEPT
   ENDIF ELSE BEGIN
      IF Keyword_Set(overplot) THEN BEGIN
         position = [!X.Window[0], !Y.Window[0], !X.Window[1], !Y.Window[1]]
...
END
!p.multi=[0,2,2]&for i=0,3 do tvimage,shift(bytscl(dist(128)),64,64)&!p.multi=0
for i=0,3 do tvimage,shift(bytscl(dist(128)),64,64),i,/tv
!p.multi=[3,3,3]&for i=0,2 do tvimage,shift(bytscl(dist(128)),64,64),marg=0&!p.multi=0
end