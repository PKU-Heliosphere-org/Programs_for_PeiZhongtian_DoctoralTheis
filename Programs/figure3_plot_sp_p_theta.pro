...
file_restore= 'slope_of_s(p)_vs_tao_theta'+'_haosan_V2.sav';;;;;;;;
...
;;--
dir_restore = 'C:\Users\pzt\course\Research\CDF_wind\'+sub_dir_date
file_restore= 'dispmodel_theta_alpha_P1'+'_V2.sav';;;;;;
file_array  = File_Search(dir_restore+file_restore, count=num_files)
For i_file=0,num_files-1 Do Begin
  Print, 'i_file, file: ', i_file, ': ', file_array(i_file)
EndFor
...
slope_mid_fast = fltarr(n_jie,n_theta)
slope_mid_slow = fltarr(n_jie,n_theta)
sigmaslope_mid_fast = fltarr(n_jie,n_theta)
sigmaslope_mid_slow = fltarr(n_jie,n_theta)
;for i_theta = 0,n_theta-1 do begin
;  for i_jie = 0,n_jie-1 do begin
    slope_mid_fast = slope_fast;median(slope_fast(*,i_jie,i_theta))
    slope_mid_slow = slope_slow;median(slope_slow(*,i_jie,i_theta))
    sigmaslope_mid_fast = sigmaslope_fast;median(sigmaslope_fast(*,i_jie,i_theta))
    sigmaslope_mid_slow = sigmaslope_slow;median(sigmaslope_slow(*,i_jie,i_theta))    
;  endfor
;endfor

A_mid_fast = fltarr(2,n_theta)
sigma_mid_fast = fltarr(2,n_theta)
A_mid_slow = fltarr(2,n_theta)
sigma_mid_slow = fltarr(2,n_theta)
;for i_liang = 0,1 do begin
;for i_theta = 0,n_theta-1 do begin
  A_mid_fast = re_A_fast;median(re_A_fast(i_liang,*,i_theta))
  sigma_mid_fast = re_sigma_fast;median(re_sigma_fast(i_liang,*,i_theta))
  A_mid_slow = re_A_slow;median(re_A_slow(i_liang,*,i_theta))
  sigma_mid_slow = re_sigma_slow;median(re_sigma_slow(i_liang,*,i_theta))
;endfor
;endfor
...
;    slope = reform(slope_fast(i_d,*,*))
;    re_A = reform(re_A_fast(*,i_d,*))
;    re_sigma = reform(re_sigma_fast(*,i_d,*))
;    sigmaslope = reform(sigmaslope_fast(i_d,*,*))
;    i_x_SubImg1  = 0
;    i_y_SubImg1  = 1
;    i_x_SubImg2  = 0
;    i_y_SubImg2  = 0    
...
;    slope = reform(slope_slow(i_d,*,*))
;    re_A = reform(re_A_slow(*,i_d,*))
;    re_sigma = reform(re_sigma_slow(*,i_d,*))
;    sigmaslope = reform(sigmaslope_slow(i_d,*,*))
;    i_x_SubImg1  = 1
;    i_y_SubImg1  = 1
;    i_x_SubImg2  = 1
;    i_y_SubImg2  = 0
...
;file_fig  = 'sp_p_and_alpha_p1_theta'+'_'+strcompress(string(i_d),/remove_all)+ $
;        '_haosan_V2.png';;
...
dy_pos_SubImg2 = (position_img(3)-position_img(1))*0.35
  ...
  for i_s = 0,1 do begin
  if i_s eq 0 then begin
    slope = slope_mid_fast
    re_A = A_mid_fast
    re_sigma = sigma_mid_fast
    sigmaslope = sigmaslope_mid_fast
    i_x_SubImg1  = 0
    i_y_SubImg1  = 1
    i_x_SubImg2  = 0
    i_y_SubImg2  = 0    
  ...
  if i_s eq 1 then begin
    re_A = A_mid_slow
    re_sigma = sigma_mid_slow    
    slope = slope_mid_slow
    sigmaslope = sigmaslope_mid_slow
    i_x_SubImg1  = 1
    i_y_SubImg1  = 1
    i_x_SubImg2  = 1
    i_y_SubImg2  = 0
...
image_tvrd  = TVRD(true=1)
dir_fig   = 'C:\Users\pzt\course\Research\CDF_wind\'+sub_dir_date
file_fig  = 'Figure4_sp_p_and_alpha_p1_theta_haosan_V2.png'
Write_PNG, dir_fig+file_fig, image_tvrd
