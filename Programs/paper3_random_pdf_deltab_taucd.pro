...
dir_restore = 'C:\Users\pzt\course\Research\CDF_wind\'+sub_dir_date
file_restore=  'random_tauc_taud_'+strfang+'_'+string(alpha)+s_v+'.sav'
file_array  = File_Search(dir_restore+file_restore, count=num_files)
For i_file=0,num_files-1 Do Begin
  Print, 'i_file, file: ', i_file, ': ', file_array(i_file)
EndFor
...
n_cell = 100.    
num_periods = 12
n_finite = dblarr(num_periods)
diff_Bz_PDF = fltarr(n_cell,num_periods)+!values.f_nan
diff_Bz_N   = fltarr(n_cell,num_periods)+!values.f_nan
diff_Bz_flat = fltarr(n_cell,num_periods)+!values.f_nan
  ...
  d_cell = max_vect(0)-min_vect(0)
  mid_vect = (min_vect+max_vect)/2.0

  i_period = 0
  n_finite(i_period) = n_elements(where(finite(npdf1)))
  ...
  diff_Bz_N(i_cell,i_period) = n_elements(sub_cell) 
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf1(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 1
  n_finite(i_period) = n_elements(where(finite(npdf2)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf2(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 2  
  n_finite(i_period) = n_elements(where(finite(npdf3)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf3(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 3
  n_finite(i_period) = n_elements(where(finite(npdf4)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_PDF(i_cell,i_period) = n_elements(sub_cell)/float(n_finite)
 ; diff_Bz_flat(i_cell,i_period) = mean(npdf4(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 4
  n_finite(i_period) = n_elements(where(finite(npdf5)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf5(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 5
  n_finite(i_period) = n_elements(where(finite(npdf6)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf6(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 6
  n_finite(i_period) = n_elements(where(finite(npdf7)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf7(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 7
  n_finite(i_period) = n_elements(where(finite(npdf8)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf8(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 8
  n_finite(i_period) = n_elements(where(finite(npdf9)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf9(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 9
  n_finite(i_period) = n_elements(where(finite(npdf10)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf10(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 10
  n_finite(i_period) = n_elements(where(finite(npdf11)))
  for i_cell = 0,n_cell-1 do begin
...
;  diff_Bz_flat(i_cell,i_period) = mean(npdf11(sub_cell)^4,/nan)*n_elements(sub_cell)/float(n_finite)
  ...
  i_period = 11
  n_finite(i_period) = n_elements(where(finite(npdf12)))
  for i_cell = 0,n_cell-1 do begin
...
n_finite = reverse(n_finite)
diff_Bz_N = reverse(diff_Bz_N,2)
diff_Bz_PDF = reverse(diff_Bz_PDF,2)
diff_Bz_flat = reverse(diff_Bz_flat,2)
;;---把耗散揉进去
SJ_Bz_N   = fltarr(n_cell,num_periods)+!values.f_nan
rem_Bz_N   = fltarr(n_cell,num_periods)+!values.f_nan
SJ_mid_vect   = fltarr(n_cell,num_periods)+!values.f_nan
for i_period = 0,2 do begin
  for i_cell = 0,n_cell-1 do begin
    rem_Bz_N(i_cell,i_period) = diff_Bz_N(i_cell,i_period)*exp(-tauc_taud(i_cell,i_period))
    SJ_mid_vect(i_cell,i_period) = mid_vect(i_cell)*exp(-tauc_taud(i_cell,i_period))
    SJ_Bz_N(i_cell,i_period) = diff_Bz_N(i_cell,i_period)-rem_Bz_N(i_cell,i_period)
  endfor
endfor

xin_Bz_N  = rem_Bz_N
xin_Bz_PDF = fltarr(n_cell,num_periods)+!values.f_nan
read,'if cut down:cut(1) or not(2)',i_cut
if i_cut eq 2 then begin
s_cut = ''  
for i_period = 0,2 do begin
  for i_cell = 0,n_cell-1 do begin
    for j_cell = 0,n_cell-1 do begin
      tex = SJ_mid_vect(i_cell,i_period)-min_vect(j_cell)
      if tex ge 0 and tex lt d_cell then begin
        xin_Bz_N(j_cell,i_period) = xin_Bz_N(j_cell,i_period) + SJ_Bz_N(i_cell,i_period)
      endif else begin
      endelse
    endfor
  endfor
...
for i_period = 3,num_periods-1 do begin
  xin_Bz_N(*,i_period) = diff_Bz_N(*,i_period)
endfor
for i_period = 0,num_periods-1 do begin
  xin_Bz_PDF(*,i_period) = xin_Bz_N(*,i_period)/float(n_finite(i_period))
endfor
endif
if i_cut eq 1 then begin
  s_cut = 'cut'
  for i_period = 3,num_periods-1 do begin
    SJ_Bz_N(*,i_period) = diff_Bz_N(*,i_period)
  endfor
  n_finite(0) = n_elements(where(finite(SJ_Bz_N(*,0))))
  n_finite(1) = n_elements(where(finite(SJ_Bz_N(*,1))))  
  n_finite(2) = n_elements(where(finite(SJ_Bz_N(*,2))))
  for i_period = 0,num_periods-1 do begin
  xin_Bz_PDF(*,i_period) = SJ_Bz_N(*,i_period)/float(n_finite(i_period))
  endfor
endif    
;;;;;新的flatness    
SJ_flat = fltarr(num_periods)
gongxian_Bz_PDF = fltarr(n_cell,num_periods)+!values.f_nan
for i_period = 0,num_periods-1 do begin
;  mean_va = total(xin_Bz_N(*,i_period)*mid_vect,/nan)/total(xin_Bz_N(*,i_period),/nan)
  std = sqrt(total(xin_Bz_N(*,i_period)*(mid_vect)^2,/nan)/total(xin_Bz_N(*,i_period),/nan))
  SJ_flat(i_period) = (total(xin_Bz_N(*,i_period)*mid_vect^4,/nan)/total(xin_Bz_N(*,i_period),/nan))/std^4
  for i_cell = 0,n_cell-1 do begin
    gongxian_Bz_PDF(i_cell,i_period) = xin_Bz_N(i_cell,i_period)*(mid_vect(i_cell)/std)^4/total(xin_Bz_N(*,i_period),/nan)
  endfor  
endfor  
...
;;;---
image_TV  = alog10(xin_Bz_PDF);alog10(diff_Bz_PDF)
...
position_SubImg = [position_img(0)+dx_pos_SubImg*(i_x_SubImg+0.1),$
           ...
           position_img(1)+dy_pos_SubImg*(i_y_SubImg+0.9)]
;;;---
image_TV  = alog10(gongxian_Bz_PDF);alog10(diff_Bz_PDF)
sub_BadVal  = Where(Finite(image_TV) eq 0)
If sub_BadVal(0) ne -1 Then Begin
  num_BadVal  = N_Elements(sub_BadVal)
  image_TV(sub_BadVal)  = !values.f_nan;0.;9999.0
...
;  XTicks=xticks,XTickV=xtickv,XTickName=xticknames,XMinor=xminor,XTickLen=0.04,$
  ...
  /NoErase,Font=-1,CharThick=1.0,Thick=1.0,/ylog
;if is_q eq 1 then begin
;xyouts, 5,100000 ,textoidl('\beta')+'='+string(alpha), color = color_black;;;;
;endif
;if is_q eq 0 then begin
;xyouts, 5,10000 ,textoidl('\alpha')+'='+string(alpha), color = color_black;;;;
;endif
;;;---
...
file_save ='random_deltaB_dissipation_'+strfang+'_'+string(alpha)+s_v+s_cut+'.sav'
dir_save  = 'C:\Users\pzt\course\Research\CDF_wind\'+sub_dir_date
data_descrip= 'got from "paper3_random_deltaB.pro"'
Save, FileName=dir_save+file_save, $
  data_descrip, $
  period_vect,xin_Bz_N,mid_vect
...
file_version= '(v1)'
file_fig  = 'random_deltaB_PDF_alpha_taucd_'+strfang+'_'+string(alpha)+s_v+s_cut+'.png';;;;;;
