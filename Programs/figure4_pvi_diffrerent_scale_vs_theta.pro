;pro figure4_PVI_diffrerent_scale_vs_theta
...
;;--
dir_restore = 'C:\Users\pzt\course\Research\CDF_wind\'+sub_dir_date
file_restore=  'LocalBG_of_MagField_for_AutoCorr_'+'10-12'+'_hao'+'_(time=*-*)(period=*-*).sav'
file_array  = File_Search(dir_restore+file_restore, count=num_files)
For i_file=0,num_files-1 Do Begin
  Print, 'i_file, file: ', i_file, ': ', file_array(i_file)
EndFor
...
period_min = 0.4;4*time_lag;耗散0.4，惯性20
period_max = 2;20*time_lag;耗散2，惯性100
...
step2:
  ...
  sub_tmp = Where((theta_arr(*,0) ge 0 and $
          theta_arr(*,0) lt 30) or (theta_arr(*,0) ge 150 and $
          theta_arr(*,0) lt 180))
  If sub_tmp(0) ne -1 Then Begin
     PVI_theta30_v1 = PVI(sub_tmp,0)      
  EndIf

  sub_tmp = Where((theta_arr(*,1) ge 0 and $
          theta_arr(*,1) lt 30) or (theta_arr(*,1) ge 150 and $
          theta_arr(*,1) lt 180))
  If sub_tmp(0) ne -1 Then Begin
     PVI_theta30_v2 = PVI(sub_tmp,1)      
  EndIf
  
  sub_tmp = Where((theta_arr(*,0) ge 60 and $
          theta_arr(*,0) lt 120))
  If sub_tmp(0) ne -1 Then Begin
     PVI_theta90_v1 = PVI(sub_tmp,0)      
  EndIf
  
  sub_tmp = Where((theta_arr(*,1) ge 60 and $
          theta_arr(*,1) lt 120))
  If sub_tmp(0) ne -1 Then Begin
     PVI_theta90_v2 = PVI(sub_tmp,1)      
  EndIf    
...
EndIf
;;;---
xtitle  = textoIDL('\delta')+'Bz/'+textoidl('\sigma')+' at 0.4s';haosan0.4s guanxing20s
ytitle  = textoIDL('\delta')+'Bz/'+textoidl('\sigma')+' at 1.2s';haosan1.2s guanxing60s
title = 'PDF'
;;;---TV image
...
xyouts,5.5,5.5,'cc='+string(cc),color=color_black
if i_theta eq 0 then begin
  cc_30 = cc
  PDF_30 = PDF
endif
...
dir_save  = 'C:\Users\pzt\course\Research\CDF_wind\'+'wind\slow\case1\'
file_save = 'PVI_scale_ralative_hao_slow_theta.sav';;改
data_descrip= 'got from "get_no_angle_sp_p.pro"'
Save, FileName=dir_save+file_save, $
  data_descrip, $
  cc_30, PVI_theta30_v1,PVI_theta30_v2, PDF_30, n_cell, $
  cc_90, PVI_theta90_v1,PVI_theta90_v2, PDF_90  



image_tvrd  = TVRD(true=1)
dir_fig   = 'C:\Users\pzt\course\Research\CDF_wind\'+'wind\slow\case1\'
file_fig  = 'PVI_scale_ralative_hao'+'_slow_theta.png';;
