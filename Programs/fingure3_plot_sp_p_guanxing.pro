;pro fingure3_plot_sp_p_guanxing
...
file_restore= 'slope_of_s(p)__guan_'+'.sav'
...
A = [1.6,0.8]
yfit = CURVEFIT(X, slope_mid_fast, weights, A, SIGMA, chisq=chisq, FUNCTION_NAME='extpmodel')
...
A = [1.6,0.8]
yfit = CURVEFIT(X, slope_mid_slow, weights, A, SIGMA, chisq=chisq, FUNCTION_NAME='extpmodel')
...
file_fig  = 'Figure3_sp_p_guanxing'+'.png'
...
;A = [1.6,0.8]
;yfit = CURVEFIT(X, slope_fast(i_d,*), weights, A, SIGMA, chisq=chisq, FUNCTION_NAME='extpmodel')
;
;plot,p,slope_fast(i_d,*),xrange=[0,9],yrange=[0.0,4],xtitle='p',ytitle=textoidl('\xi')+'(p)', $
;  xthick=2,ythick=2, xstyle = 1,ystyle = 1, $
;  color=color_black,position=position_SubImg,charsize=1.5,charthick = 2,thick=2,/noerase
;oplot, p,Y, color = color_red, linestyle =2,thick=2  
;;ErrPlot, X , slope_mid_fast - ssigma, s + ssigma, color=color_black,Thick=4, width = 0.04
;print,'Function parameters: ',A
;bx1 = Alog10(A(1)^(X/3)+(1-A(1))^(X/3))/alog10(2)
;F1 = (-2.5+1.5*A(0))*(X/3)+1-bx1
...
;A = [1.6,0.8]
;yfit = CURVEFIT(X, slope_slow(i_d,*), weights, A, SIGMA, chisq=chisq, FUNCTION_NAME='extpmodel')
;plot,p,slope_slow(i_d,*),xrange=[0,9],yrange=[0.0,4],xtitle='p',ytitle=textoidl('\xi')+'(p)', $
;  xthick=2,ythick=2, xstyle = 1,ystyle = 1, $
;  color=color_black,position=position_SubImg,charsize=1.5,charthick = 2,thick=2,/noerase
;oplot, p,Y, color = color_red, linestyle =2,thick=2  
;;ErrPlot, X , slope_mid_fast - ssigma, s + ssigma, color=color_black,Thick=4, width = 0.04
...
;image_tvrd  = TVRD(true=1)
;dir_fig   = 'C:\Users\pzt\course\Research\CDF_wind\'+sub_dir_date
;file_fig  = 'Figure3_sp_p_guanxing_'+strcompress(string(i_d+1),/remove_all)+'_.png'
;Write_PNG, dir_fig+file_fig, image_tvrd
;
;endfor
