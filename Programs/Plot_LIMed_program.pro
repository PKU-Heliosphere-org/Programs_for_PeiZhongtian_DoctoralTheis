...
sub_dir_date  = 'new\19950226-0301\'
...
file_restore= 'uy_1sec_vhm_19950226-0301_v01.sav'
...
If i_Bcomp eq 4 Then FileName_BComp='Bt'
...
step2:
PSD_BComp_time_scale_arr = real_part(BComp_wavlet_arr)
...
PSD_time_freq_F = mean(PSD_time_freq^4.)/(mean(PSD_time_freq^2))^2.
PSD_time_freq_I = PSD_time_freq^2./mean(PSD_time_freq^2.)
...
if i_iter ne 0 then oplot,JulDay_2s_vect,fltarr(n_time)+iter_value
...
;  iter_value = iter_value_arr(i_iter)
  ...
  if iter_value gt 0 then begin
    ...
    if sub_posi(n_elements(sub_posi)-1) ne n_time-1 then begin
      ...
      JulDay_vect_v1 = [JulDay_vect_v1,JulDay_2s_vect(n_time-1)]
    endif
...
;    BComp_wavlet_time_v2 = complex(BComp_wavlet_time_v2_real,BComp_wavlet_time_v2_imag)
    BComp_wavlet_time_v2 = interpol(BComp_wavlet_time_v1,JulDay_vect_v1,JulDay_2s_vect)
...
;file_fig  = FileName_BComp+'LIM_Flat_'+string(i_freq,format='(I2)')+file_version+'.png'
;Write_PNG, dir_fig+file_fig, image_tvrd

window,1,xs=1200,ysize = 750.0/2
plot,JulDay_2s_vect,real_part(BComp_wavlet_arr_ori(*,i_freq)),XRange=xrange,XStyle=1,$
  XTicks=xticks,XTickV=xtickv,XTickName=xticknames,XMinor=xminor,XTickLen=0.04,$
  XTitle=xtitle,YTitle='Wavelet Coefficient (km/s)',color=color_black
oplot,JulDay_2s_vect,real_part(BComp_wavlet_arr(*,i_freq)),color=color_red
oplot,JulDay_2s_vect,real_part(BComp_wavlet_arr(*,i_freq))-real_part(BComp_wavlet_arr_ori(*,i_freq)),color=color_green
...
Save, FileName=file_save, $
    data_descrip, $
    JulDay_2s_vect, time_vect_v2, period_vect, $
