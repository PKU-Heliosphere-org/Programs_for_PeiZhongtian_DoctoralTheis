...
;pro extpmodel2, X,A,F,pder;对这个公式有疑问，拟合很费劲
;bx = Alog10(A(2)^(X/3)+(1-A(2))^(X/3))/alog10(2)
;F = A(1)*((-2.5+1.5*A(0))+(1-bx))
;pder = [[replicate(1.5*A(1),N_ELEMENTS(X))],[(-2.5+1.5*A(0))+(1-bx)],[-A(1)*(A(0)-1)*A(2)*(A(2)^(X/3-1)-(1-A(2))^(X/3-1))/(2*Alog(2)*(A(2)^(X/3)+(1-A(2))^(X/3)))]]
;End

pro extpmodel2, X,A,F,pder
bx = Alog10(A(2)^(X/3)+(1-A(2))^(X/3))/alog10(2)
F = A(1)*((-2.5+1.5*A(0))*(X/3.0)+(1-bx))
pder = [[1.5*A(1)*X/3.0],[(-2.5+1.5*A(0))*(X/3.0)+(1-bx)],[-A(1)*(A(0)-1)*A(2)*(A(2)^(X/3-1)-(1-A(2))^(X/3-1))/(2*Alog(2)*(A(2)^(X/3)+(1-A(2))^(X/3)))]]
end
...
;;;--
Set_Plot, 'PS';'Win'
Device,filename=dir_restore+'sp_p_no_theta_2.eps', $   ;
...
X = (findgen(n_jie)+1.0)/2.0   ;0.5-10.0阶
weights = 1.0/s

;we = fltarr(n_jie)+1.0
;yfit = fltarr(20)
 yfit = CURVEFIT(X,s, weights, A, SIGMA,chisq=chisq, FUNCTION_NAME='extpmodel2', ITmax=60)
plot,X,s,xrange=[0,11],yrange=[0.0,4.0],xtitle='p',ytitle=textoidl('\xi')+'(p)', $
  color=color_black,position=position_SubImg,charthick = 2,thick=2,title = title,/noerase
...
xyouts,1,3.5,textoidl('\alpha')+' = '+string(A(0),format='(F4.2)')+' '+textoIDL('\pm')+' '+string(sigma(0),format='(F4.2)'),charsize=0.6,charthick=2
xyouts,1,3.2,textoidl('P_1')+' = '+string(A(1),format='(F4.2)')+' '+textoIDL('\pm')+' '+string(sigma(1),format='(F4.2)'),charsize=0.6,charthick=2
...
file_save = 'extpmodel_theta_alpha_P1_recon_1.sav'
