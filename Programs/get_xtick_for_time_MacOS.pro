Pro get_xtick_for_time_MacOS, xrange_time, $
...
If JulDay_range gt 5.0 Then Begin
	...
	xminor_time		= 4
EndIf Else Begin
if julday_range ge 2.0 then begin
  djulday_tick = double(8.)/24
  xminor_time   = 4
endif else begin
...
If JulDay_range gt 2.*(1./24) Then Begin
  dJulDay_tick  = Double(2.)/24;Double(1./2)/24
  xminor_time   = 3
EndIf Else Begin
If JulDay_range gt (1/2.)*(1./24) Then Begin
	dJulDay_tick	= Double(1./6.)/24;Double(1./2)/24
	xminor_time		= 3
EndIf Else Begin
If JulDay_range gt 1./(24*60)*10. Then Begin
;a	dJulDay_tick	= Double(1.)/(24*60)*10.
;a 	xminor_time		= 10
    dJulDay_tick  = Double(1.)/(24*60)*2
    xminor_time = 2
EndIf Else Begin
