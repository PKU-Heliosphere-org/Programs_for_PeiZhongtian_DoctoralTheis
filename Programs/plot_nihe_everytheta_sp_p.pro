...
plot,[0],[0],xrange=[0,11],yrange=[0.0,8.0];,/isotropic
ds=findgen(num_theta_bins)*0.1
p=(indgen(n_jie)+1)/2.0
slope=fltarr(num_theta_bins,n_jie)
bx=fltarr(n_jie)
...
for i=0,num_theta_bins-1 do begin
  ...
  for j=0,n_jie-1 do begin
    bx(j) = Alog10(re_A(1,i)^(p(j)/3.0)+(1.0-re_A(1,i))^(p(j)/3.0))/alog10(2.0)
  slope(i,j) = (-2.5+1.5*re_A(0,i))*(p(j)/3.0)+1.0-bx(j)
  if finite(slope(i,j)) eq 0 then begin
    con=0
  endif 
  endfor
  if con eq 1 then begin
  plot,p,slope(i,*)+ds(i),xrange=[0,11],yrange=[0.0,8.0],xtitle='p',ytitle='s',/noerase;,/isotropic
  endif
  con=1
endfor
...
;file_version= '(v1)'
file_fig  = 'slope_nihe_of_s(p)_vs_theta_tao'+$
        '.png'
...
;;--
AnnotStr_tmp  = 'got from "plot_everytheta_sp_p.pro"'
