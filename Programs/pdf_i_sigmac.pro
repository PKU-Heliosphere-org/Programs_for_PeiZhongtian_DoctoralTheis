...
for i_cell = 0,n_cell-1 do begin
  PDF(j_cell,i_cell) = n_elements(where((I_plot ge (I_min+j_cell*I_bin) and I_plot le (I_min+(j_cell+1)*I_bin)) $
    and (C_plot ge (C_min+i_cell*C_bin) and C_plot le (C_min+(i_cell+1)*C_bin)) ))
endfor
endfor
